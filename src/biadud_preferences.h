...
#if !defined(BIADUD_PREFERENCES_H)
#define BIADUD_PREFERENCES_H
...
struct preferences {
	char    signature[9]    		= "BIADUD00";		// Signature for this data format
...
//	char    sms_command[9]  		= "";				// Par défaut on met toujours les coordonnées dans le SMS envoyé
	...
	char	ip_localaddress[16]		= "8.8.4.4";		// The default IP android DNS
	char	ip_gatewayaddress[16]	= "8.8.4.4";		// The default IP android DNS
...
//	int logAfter = 5 ;   // enregistrement d'un point si déplacement de 5m  (si <0, enregistrement après n millis sec)
	int		gps_baud				= 9600;				// GPS serial transmission speed in Bauds
	byte	gps_rate				= 1;				// GPS refresh rate in Hertz
...
//#define DEBUG_HEAP true
	...
	#define dbgHeap(mes)\
		do {\
			if (DEBUG_HEAP) {\
				Serial.printf_P(	PSTR("\n<<<%s %s():%d  FreeContStack: %5d  free: %5d - max: %5d - frag: %3d%%>>>\n "),\
									mes,\
									__func__,\
									__LINE__,\
...
struct preferences  preferences_factory;	// Custom preferences
struct preferences  preferences_custom;		// Default factory preferences
...
#endif // #if !defined(BIADUD_PREFERENCES_H)
